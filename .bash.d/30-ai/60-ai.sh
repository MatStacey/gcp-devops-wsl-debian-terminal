...
__ai_build_context() {
	...
	if [ -n "$target_file" ]; then
		[ ! -f "$target_file" ] && { echo "🚨 Error: Target context file '$target_file' not found." >&2; return 1; }
...
__ai_query_gemini() {
	...
	[ -z "${GEMINI_API_KEY}" ] && { echo "🚨 Error: GEMINI_API_KEY is not set. Run 'add-gemini-key'." >&2; return 1; }
	...
	[ -z "$content" ] && { echo "🚨 Error: Failed to get a valid response from Gemini." >&2; echo "$response" | jq -r '.error.message // "Unknown error"' >&2; return 1; }
	echo "$content"
}
...
__ai_query_claude() {
	...
	[ -z "${CLAUDE_API_KEY}" ] && { echo "🚨 Error: CLAUDE_API_KEY is not set. Run 'add-claude-key'." >&2; return 1; }
	...
	[ -z "$content" ] && { echo "🚨 Error: Failed to get a valid response from Claude." >&2; echo "$response" | jq -r '.error.message // "Unknown error"' >&2; return 1; }
	echo "$content"
}
...
__ai_save_output() {
	...
	if [ -n "$explicit_out_file" ]; then
		...
		case "$category" in
		gcloud) target_dir="${AI_WORKSPACE_DIR}/generated/gcloud"; base_name="${target_dir}/${final_title}" ;;
		script) target_dir="${AI_WORKSPACE_DIR}/generated/scripts/${lang}"; base_name="${target_dir}/${final_title}" ;;
		project | *) target_dir="${AI_WORKSPACE_DIR}/generated/projects"; base_name="${target_dir}/${final_title}-${lang}" ;;
		esac
...
ai() {
	...
	while getopts "m:t:ef:o:v:x" opt; do
		...
		x) req_extended=true ;;
		\?) echo "Usage: ai [-m gemini|claude] [-t title] [-e] [-f file] [-o out_file] [-v version] [-x] <prompt>" >&2; return 1 ;;
		esac
	done
	shift $((OPTIND - 1))
	prompt="$*"

	[ -z "${prompt}" ] && { echo "Usage: ai [-m gemini|claude] [-t title] [-e] [-f file] [-o out_file] [-v version] [-x] <your question>" >&2; return 1; }
