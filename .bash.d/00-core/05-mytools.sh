...
mt-cat() {
	[[ "$1" == "-h" || "$1" == "--help" ]] && { mt-help "${FUNCNAME[0]}"; return 0; }
	[ -z "$1" ] && { echo "Usage: mt-cat <category>"; mt-cats; return 1; }
...
mt-search() {
	[[ "$1" == "-h" || "$1" == "--help" ]] && { mt-help "${FUNCNAME[0]}"; return 0; }
	[ -z "$1" ] && { echo "Usage: mt-search <keyword>"; return 1; }
	mytools >/dev/null
...
mt-help() {
	[[ "$1" == "-h" || "$1" == "--help" ]] && { echo "Usage: mt-help <command>"; return 0; }
	local target="$1"
	[ -z "$target" ] && { echo "Usage: mt-help <command>"; return 1; }
