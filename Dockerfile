...
# Reason: KICS falsely flags 'apt' in the Google Cloud URL as the 'apt' CLI command. 
# Reason: Pinning Debian apt packages causes build rot when upstream mirrors purge old versions.
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    google-cloud-cli terraform packer \
...
# (Owned by root; rsync in the install script handles the final user ownership)
COPY .bashrc install.sh /tmp/gcp-devops-terminal/
COPY .bash.d/ /tmp/gcp-devops-terminal/.bash.d/

# 6. Install Python tooling and run the profile installer in a single layer
RUN pipx install ruff \
    && pipx install checkov \
    && pipx install yapf \
    && cd /tmp/gcp-devops-terminal \
    && echo "n" | ./install.sh \
    && rm -rf /tmp/gcp-devops-terminal \
